...
from PyQt6.QtWidgets import QApplication, QScrollArea, QSizePolicy  # noqa: E402
...
@pytest.fixture
def qt_app():
    """Provide the single QApplication a widget needs to exist.

    Returns:
        QApplication: The running instance.
    """
...
def test_the_panel_reports_the_labels_on_disk(project_sandbox, qt_app):
...
def test_refreshing_replaces_the_rows_rather_than_stacking_them(project_sandbox, qt_app):
...
def test_a_class_added_while_the_window_is_open_appears_on_refresh(project_sandbox, qt_app):
...
def test_a_project_with_no_classes_shows_a_hint_instead_of_an_empty_box(project_sandbox, qt_app):
...
def test_the_last_pipeline_button_can_be_scrolled_to(project_sandbox, qt_app):
...
def test_the_dataset_panel_is_not_inside_the_scrolling_part(project_sandbox, qt_app):
...
def test_the_panel_takes_only_the_height_it_needs(project_sandbox, qt_app):
